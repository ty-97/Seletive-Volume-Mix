...
def path_trans(orig_path):
    file = orig_path.split('/')[-1]
    dir = orig_path.split('/')[:-1]
...
class Ssv1(torch.utils.data.Dataset):
    ...
    def _construct_loader(self):
        ...
        with PathManager.open(
            ...
            label_dict = {label: idx for idx,label in enumerate(f)}
        ...
        with PathManager.open(label_file, "r") as f:
            label_json = [{"id":vid_label.split(';')[0],"template":vid_label.split(';')[1]} for vid_label in f]
        ...
        self._path_to_videos, _ = utils.load_image_lists_v1(
    ...
    def __getitem__(self, index):
        ...
        # vid_dir = os.path.join('/',*self._path_to_videos[index][0].split('/')[:-1])
        # video_length = len(os.listdir(vid_dir))
        video_length = int(self._path_to_videos[index][1])
        ...
        for i in range(num_frames):
            ...
            if self.mode == "train":
                ...
                seq.append((start + end) // 2 + 1)
        #print(seq)
        frames = torch.as_tensor(
            utils.retry_load_images(
                [os.path.join(self._path_to_videos[index][0],'{:05d}.jpg'.format(frame)) for frame in seq],
                #[path_trans(self._path_to_videos[index][frame]) for frame in seq],
                self._num_retries,
            )
        )
