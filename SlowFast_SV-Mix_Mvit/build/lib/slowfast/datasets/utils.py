...
def load_image_lists_v1(frame_list_file, prefix="", return_list=False):
    """
    Load image paths and labels from a "frame list".
    Each line of the frame list contains:
    `original_vido_id video_id frame_id path labels`
    ...
    with PathManager.open(frame_list_file, "r") as f:
        ...
        for i,line in enumerate(f):
            ...
            row = line.split()
            # if len(row) != 5:
            #     print(row,len(row),i,line)
            #     continue
            #print(len(row))
            #original_vido_id video_id frame_id path labels
            #assert len(row) == 5
            video_name = row[0].split('/')[-1]
            if prefix == "":
                path = row[0]
            else:
                path = os.path.join(prefix, row[0])
            image_paths[video_name].append(path)
            image_paths[video_name].append(row[1])
            frame_labels = row[-1].replace('"', "")
            if frame_labels != "":
                labels[video_name].append(
                    [int(x) for x in frame_labels.split(",")]
