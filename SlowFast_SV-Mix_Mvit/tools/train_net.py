...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, index, time, meta) in enumerate(
        ...
        if cfg.MIXUP.ENABLE:
            #print(type(inputs[0]),type(labels))
            #print('119',labels[0].device)

            samples, labels, x_ofl, label_ofl, mask_loss = mixup_fn(inputs[0], labels[0])
            #print('121',labels[0].device)
            inputs[0] = samples
            x_ofl = [x_ofl] 
            #print(len(inputs),inputs[0].shape)
            #print(samples.shape,'sample')
        ...
        with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            ...
            if cfg.MODEL.MODEL_NAME == "ContrastiveModel":
                ...
                #print('before send to net',inputs.shape)
                preds = model(inputs)
            ...
            if cfg.MODEL.MODEL_NAME == "ContrastiveModel" and partial_loss:
                ...
                # Compute the loss.
                #print('156',labels.device)
                labels = labels.cuda(non_blocking=True)
                loss = loss_fun(preds, labels)

                label_ofl = label_ofl.cuda(non_blocking=True)
...
def train(cfg):
    ...
    if cfg.TRAIN.AUTO_RESUME and cu.has_checkpoint(cfg.OUTPUT_DIR):
        ...
        if last_checkpoint is not None:
            ...
            cp = torch.load(last_checkpoint, map_location="cpu")
            cp_ofl = cp['model_ofl']
            mix_block = cp['mix_block']
            if cfg.NUM_GPUS > 1:
                mixup_fn.model_ofl.module.load_state_dict(cp_ofl)
                mixup_fn.mix_block_s.module.load_state_dict(mix_block)
            else:
                mixup_fn.model_ofl.load_state_dict(cp_ofl)
                mixup_fn.mix_block_s.load_state_dict(mix_block)
    ...
    if cfg.TENSORBOARD.ENABLE and du.is_master_proc(
        ...
        writer = None #tb.TensorboardWriter(cfg)
    else:
        writer = None
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        if is_eval_epoch or cur_epoch==10:
            eval_epoch(
                val_loader,
                # model,
                mixup_fn.model_ofl,
