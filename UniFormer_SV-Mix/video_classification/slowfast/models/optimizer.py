...
def construct_optimizer(model,mix_block_s, cfg):
    """
    ...
    for m in mix_block_s.modules():
        if isinstance(m, (torch.nn.Conv1d, 
            ...
            new_para.append(ps[0])
            if len(ps) == 2:
                new_para.append(ps[1])

    optim_params = [
        {
                "params": new_para,
                #'lr': 5*cfg.SOLVER.BASE_LR,
                "weight_decay": cfg.SOLVER.WEIGHT_DECAY,
                "layer_decay": 5.0, # tocheck
                "apply_LARS": False,
            },
    ...
    assert len(list(model.parameters()))+len(list(mix_block_s.parameters())) == len(non_bn_parameters) + \
                                                                                len(bn_parameters) + \
                                                                                len(new_para) + \
        ...
        len(bn_parameters),
        len(zero_parameters),
        len(list(model.parameters()))+len(list(mix_block_s.parameters())),
    )
    # print(
    ...
    # )
    print(
            "bn {}, non bn {}, zero {}, new para {}".format(
                len(bn_parameters),
                len(non_bn_parameters),
                len(zero_parameters),
                len(new_para)
...
def set_lr(optimizer, new_lr):
    ...
    for param_group in optimizer.param_groups:
        ...
        # print(param_group)
        param_group["lr"] = new_lr * param_group["layer_decay"] if 'layer_decay' in param_group.keys() else new_lr 
