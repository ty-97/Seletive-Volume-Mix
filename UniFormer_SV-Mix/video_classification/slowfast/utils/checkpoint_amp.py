...
def save_checkpoint(path_to_job, model,model_ofl,mix_block, optimizer, loss_scaler, epoch, cfg):
    """
    Save a checkpoint.
    Args:
    ...
    checkpoint = {
        ...
        "model_ofl": model.module.state_dict() if cfg.NUM_GPUS > 1 else model.state_dict(),
        "mix_block": mix_block.module.state_dict() if cfg.NUM_GPUS > 1 else model.state_dict(),
...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        for k in model_state.keys():
            if '_orig_mod.' in k:
                new_model_state[k.replace('_orig_mod.','')] = model_state[k]
            else:
                new_model_state[k] = model_state[k]
...
def load_test_checkpoint(cfg, model):
    ...
    if cfg.TEST.CHECKPOINT_FILE_PATH != "":
        ...
        load_checkpoint(best_checkpoint, model, None, cfg.NUM_GPUS > 1,test=True,)
    elif has_checkpoint(cfg.OUTPUT_DIR):
        last_checkpoint = get_last_checkpoint(cfg.OUTPUT_DIR)
        load_checkpoint(last_checkpoint, model, None, cfg.NUM_GPUS > 1,test=True,)
...
def load_train_checkpoint(cfg, model, optimizer, loss_scaler,mixup_fn=None):
    """
