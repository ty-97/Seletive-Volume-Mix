...
from slowfast.utils import comm
...
def create_ddp_model(model, cfg, gpu_id=None):
    """
...
class MB_st(nn.Module):
    ...
    def forward(self,x,lam,index,scale=32,use_cutmix=True):
        if use_cutmix:
            ...
            ).view(n,t,1,1,h,w).expand(n,t,2,1,h,w).reshape(n*t*2,1,h,w)

            mask_lam_ = F.interpolate(mask_lam_, scale_factor=scale, mode='nearest')
            h_n,w_n = mask_lam_.shape[-2:]
            mask_lam_ = mask_lam_.reshape(n,1,t*2,h_n,w_n)

            mask_lam_ = torch.sigmoid(mask_lam_.type(torch.float32))
            mask = torch.cat([1 - mask_lam_, mask_lam_], dim=1) 
            #print(mask.shape)
            return mask,v_.reshape(n,t,1,h,w).expand(n,t,2,h,w).reshape(n,2*t,h,w)
        else:
            n, t, c, h, w = x.size()
            ...
            mask_lam_ = torch.matmul(
                pairwise_weight.type(torch.float32), v_.type(torch.float32)
            ).view(n, t,1,1,1).expand(n,t,2,h,w).reshape(n*2*t,1,h,w)
            ...
            return mask,v_.reshape(n,t,1,1,1).expand(n,t,2,1,1).reshape(n,2*t,1,1)
...
def mask_loss_func(mask, lam):
    ...
    assert mask.dim() == 5
    n, k, t, h, w = mask.size()  # mixup mask [N, 2, H, W]
    if k > 1:  # the second mask has no grad!
        mask = mask[:, 1, :, :].unsqueeze(1)
    m_mean = mask.sum() / (n * t* h * w)  # mask mean in [0, 1]
...
class MaskMotionCut:
    ...
    def __init__(
        ...
        self.model_ofl = model
        #self.model_ofl.eval()
        self.model_ofl.requires_grad_(False)
        #self.model_ofl.eval()
        for n,m in self.model_ofl.named_modules():
        ...
        if cfg.MODEL.ARCH == 'mvit':
            mix_box_s = MB_st(in_channel=768,inter_channel=768,)
        elif cfg.MODEL.ARCH == 'uniformer':
            mix_box_s = MB_st(in_channel=512,inter_channel=512,)
        # mix_box_s = torch.compile(mix_box_s)
        self.mix_block_s = create_ddp_model(mix_box_s, cfg)
        ...
        self.momentum = 0.999
        #self.weight = 0
        #print(self.model_ofl.module.conv1.weight[0,0,0,:])
        self.category = [i.replace('\n','') for i in open('/data/tanyi/some_some_v1/category.txt')]
    ...
    def vis(self,vids,vids_ori,value,mask,target,index,lam,type,batch):
        ...
        vids = vids.transpose(1,2).reshape(b,-1,h,w)
        vids_ori = vids_ori.transpose(1,2).reshape(b,-1,h,w)

        mask = mask.transpose(1,2).reshape(b*t,2,h,w)
    ...
    def momentum_update(self, model):
        ...
        # print(self.model_ofl.device,'\n\n\n')
        #print('momentum_update')
        for p_i, p_j in zip(self.model_ofl.parameters(),model.parameters()):
    ...
    def _mix_batch(self, x, target):
        ...
        with torch.no_grad():
            _,feature_map = self.model_ofl(x_clone,return_fea_map=True)
        feature_map = feature_map.detach()

        # if x.ndim == 4:
        #     b, c, h, w = x.shape
        #     x = x.reshape(b, c // 3, 3, h, w).reshape(-1,3,h,w) #.permute(0, 2, 1, 3, 4)
        #print(x.shape)
        b,c,t,h,w = x.shape
        lam_ofl = np.random.beta(self.cutmix_alpha, self.cutmix_alpha)

        b, t_f, h_f, w_f,c_f =feature_map.shape
        
        feature_map = feature_map.permute(0,1,4,2,3)#.reshape(b, c // 3, c_f, h_f, w_f)
        ...
        mask_ol_s = self.mix_block_s(feature_map,lam,rand_index_ol,h//h_f,use_cutmix)[0].clone().detach()  # b 2 2t_f h w 
        mask_ofl_s,value_s = self.mix_block_s(feature_map,lam_ofl,rand_index_ofl,h//h_f,use_cutmix)
        ...
        x_ofl = x * mask_ofl[:, 0:1] + x[rand_index_ofl] * mask_ofl[:, 1:]
        x_ol = x * mask_ol[:, 0:1] + x[rand_index_ol] * mask_ol[:, 1:]
        #x_ol = x * mask_ol[:, 0, :, :].unsqueeze(1) + x.reshape(b,-1,3,h,w)[rand_index_ol, :].reshape(-1,3,h,w) * mask_ol[:, 1, :, :].unsqueeze(1)

        # x_ofl = x_ofl.reshape(b, c, h, w)
        # x_ol = x_ol.reshape(b, c, h, w)
        ...
        saliency = self.softmax(torch.cat([feature_map,feature_map],dim=1).mean(2).reshape(b,-1)).reshape(b,t_f*2,h_f,w_f)
