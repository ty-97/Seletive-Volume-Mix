...
def saliency_mo_bbox(vid, lam): #img size b c h w
    size = vid.size()
    b,c,t,h,w = size
    ...
    diff_vid = (vid - vid.mean(dim=2,keepdim=True)).abs().mean(dim=[1,2]) # b h w 

    cor = torch.argmax(diff_vid.reshape(b,h*w),dim=1) #b
...
class MotionCutStatic:
    ...
    def _mix_batch(self, x):
        ...
        if use_cutmix:
            ...
            crop_cor, lam = saliency_mo_bbox(x[rand_index], lam) # b t h_box w_box 2
            #print(type(lam))
