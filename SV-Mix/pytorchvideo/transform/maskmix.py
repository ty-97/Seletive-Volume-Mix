...
class MaskMix:
    ...
    def _mix_batch(self, x):
        ...
        if use_cutmix:
            ...
            # print(x.shape)
            last_sp = [8, 7, 7]
            x = x.view(b, c, last_sp[0], t // last_sp[0], last_sp[1], h // last_sp[1], last_sp[2], w // last_sp[2])
            # x.reshape_(b,t,c,last_sp[1],h//last_sp[1],last_sp[2],w//last_sp[2])

            num_mask = int(last_sp[0] * last_sp[1] * last_sp[2] * (1 - lam))  # num of mask
            num_remain = last_sp[0] * last_sp[1] * last_sp[2] - num_mask
            lam = num_remain / (last_sp[0] * last_sp[1] * last_sp[2])

            mask = np.hstack([np.ones(num_mask), np.zeros(num_remain)])
            np.random.shuffle(mask)
            mask = np.array(mask, dtype='bool')  # .reshape(*last_sp)

            full_cor = np.linspace(0, last_sp[0] * last_sp[1] * last_sp[2] - 1, last_sp[0] * last_sp[1] * last_sp[2])
            mask_cor = full_cor[mask]
            t_cor = mask_cor // (last_sp[1] * last_sp[2])
            h_cor = (mask_cor % (last_sp[1] * last_sp[2])) // last_sp[1]
            w_cor = (mask_cor % (last_sp[1] * last_sp[2])) % last_sp[1]

            # x[:,:,t_cor,h_cor,:,w_cor,:] = x.flip(0)[:,:,t_cor,h_cor,:,w_cor,:]
            x[:, :, t_cor, :, h_cor, :, w_cor, :] = x.flip(0)[:, :, t_cor, :, h_cor, :, w_cor, :]
            # print(x.shape,'during cutmix')
            # print()
