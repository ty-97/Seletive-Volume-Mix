...
def test_correlation_zeropad():
    ...
    y1 = correlation_zeropad(x, w, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation,
                             group_size=c_x)
...
def test_gradcheck():
    ...
    w = torch.randn(c_x, in_time, kernel_size, kernel_size, requires_grad=True).double().cuda()
    from functools import partial
    y = correlation_zeropad(x, w, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation,
                            group_size=group_c)
    assert torch.autograd.gradcheck(partial(correlation_zeropad, kernel_size=kernel_size, 
