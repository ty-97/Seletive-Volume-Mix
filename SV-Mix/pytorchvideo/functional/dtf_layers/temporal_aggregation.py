...
from pytorchvideo.utils.dtf_utils import Dtype, Stream, load_kernel
...
# NCHW -> channel-wise aggregation by attention weights
_aggregation_forward_kernel = kernel_loop + '''
extern "C"
__global__ void aggregation_forward_kernel(
...
_aggregation_input_backward_kernel = kernel_loop + '''
extern "C"
__global__ void aggregation_input_backward_kernel(
...
_aggregation_weight_backward_kernel = kernel_loop + '''
extern "C"
__global__ void aggregation_weight_backward_kernel(
...
class Aggregation(Function):
    ...
    def forward(ctx, input, weight, kernel_size, stride, padding, dilation):
        ...
        with torch.cuda.device_of(input):
            f = load_kernel('aggregation_forward_kernel', _aggregation_forward_kernel, Dtype=Dtype(input), nthreads=n,
    ...
    def backward(ctx, grad_output):
        ...
        with torch.cuda.device_of(input):
            if ctx.needs_input_grad[0]:
                ...
                n = grad_input.numel()
                opt['nthreads'] = n
                f = load_kernel('aggregation_input_backward_kernel', _aggregation_input_backward_kernel, **opt)
                f(block=(CUDA_NUM_THREADS, 1, 1),
                  grid=(GET_BLOCKS(n), 1, 1),
            ...
            if ctx.needs_input_grad[1]:
                ...
                opt['nthreads'] = n
                f = load_kernel('aggregation_weight_backward_kernel', _aggregation_weight_backward_kernel, **opt)
                f(block=(CUDA_NUM_THREADS, 1, 1),
                  grid=(GET_BLOCKS(n), 1, 1),
...
def aggregation(input, weight, kernel_size=3, stride=1, padding=0, dilation=1):
    assert input.shape[0] == weight.shape[0] and (input.shape[1] % weight.shape[2] == 0)
    if input.is_cuda:
        out = Aggregation.apply(input, weight, kernel_size, stride, padding, dilation)
    else:
        out = Aggregation.apply(input.cuda(), weight.cuda(), kernel_size, stride, padding, dilation)
...
def test_aggregation():
    ...
    y1 = aggregation(x, w, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation)
    ...
    assert torch.autograd.gradcheck(partial(aggregation, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation), (x, w))
    print('temporal aggregation: test case passed.')


def test_aggregation_ks1():
    ...
    from functools import partial
    assert torch.autograd.gradcheck(partial(aggregation, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation), (x, w))
    print('temporal aggregation: test_1ks case passed.')


if __name__ == '__main__':
    print('--------- temporal aggregation testing ---------')
    test_aggregation()
    test_aggregation_ks1()
    print('--------------------- Done ---------------------')
