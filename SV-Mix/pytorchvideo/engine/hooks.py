...
from pytorchvideo.checkpoint import PeriodicEpochCheckpointer as _PeriodicCheckpointer
from pytorchvideo.evaluation.testing import flatten_results_dict
from pytorchvideo.utils.events import EventStorage, EventWriter
...
class PeriodicCheckpointer(_PeriodicCheckpointer, HookBase):
    ...
    def after_train(self):
        logger = logging.getLogger(__name__)
        logger.info(f'Best accuracy {self.trainer.best_score:.3%}')

    def state_dict(self):
        ret = super().state_dict()
        ret["score"] = getattr(self, "score", 0)
        return ret
...
class AutogradProfiler(HookBase):
    ...
    def after_step(self):
        ...
        if "://" not in out_file:
            ...
            with tempfile.TemporaryDirectory(prefix="pytorchvideo_profiler") as d:
...
class EvalHook(HookBase):
    ...
    def __init__(self, eval_period, dense_eval_epoch, eval_function, iters_per_epoch, stage='Test'):
        """
        Args:
        ...
        self._dense_eval_iter = dense_eval_epoch * iters_per_epoch
    ...
    def after_step(self):
        ...
        if (self._period > 0) and (next_iter % self._period == 0 or (next_iter > self._dense_eval_iter and next_iter % self._iters_per_epoch == 0)):
            self._do_eval(epoch)
