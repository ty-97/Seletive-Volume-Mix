...
class SIFASwinTransformerBlock(SwinTransformerBlock):
    ...
    def forward_rtc(self, x):
        ...
        x = x.view(n_batch, self.clip_len, C, self.input_resolution[0],
                   self.input_resolution[1]).transpose(1, 2)  # NCTHW

        x_tmp = x.clone()
        x_tmp[:, :, 1:, :, :] = x_tmp[:, :, :-1, :, :]
        x_tmp = (torch.sigmoid(x - x_tmp) * x) + x
        # offset = self.conv_offset(x_tmp)
        offset = nn.Parameter(torch.zeros(n_batch, self.off_channels_, self.clip_len, self.input_resolution[0],
                                          self.input_resolution[1])).cuda()
...
class SIFA_SWIN_ViT(C2D_SWIN_ViT):
    _arch_dict = {
        # SWIN-ViT model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
        'sifa_swin_vit_s_p4_w7': dict(drop_path_rate=0.3, window_size=7, patch_size=4, in_chans=3, embed_dim=96,
                                      depths=[2, 2, 18, 2], num_heads=[3, 6, 12, 24], cor_dilation=1,
                                      blocks=[SIFABasicLayer, SIFABasicLayer, SIFABasicLayer, SIFABasicLayer]),
        'sifa_swin_vit_b_p4_w7': dict(drop_path_rate=0.5, window_size=7, patch_size=4, in_chans=3, embed_dim=128,
                                      depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32], cor_dilation=1,
                                      blocks=[SIFABasicLayer, SIFABasicLayer, SIFABasicLayer, SIFABasicLayer]),
        'sifa_swin_vit_l_p4_w7': dict(drop_path_rate=0.5, window_size=7, patch_size=4, in_chans=3, embed_dim=192,
    ...
    def __init__(self, blocks=[], img_size=224, early_stride=4, patch_size=4, in_chans=3, num_classes=1000,
        ...
        Args:
            ...
            depths (list): depth of transformer
    ...
    def from_config(cls, cfg):
        ...
        sifa_kernel = arch_cfgs[0] if (len(arch_cfgs) > 0 and isinstance(arch_cfgs[0], int)) else None
        K = 3
        if sifa_kernel:
            K = sifa_kernel
