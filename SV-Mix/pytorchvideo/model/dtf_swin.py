...
from .dtf_resnet import FFTLMul, FFTLinear, FFTBatchNorm
...
class DTFSwinTransformerBlock(SwinTransformerBlock):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, K=3, cor_dilation=1, cor_group=1, clip_len=4,
                 fix_offset=True):
    ...
    def forward_dtf(self, x):
        # -------------- Reshape input feature to N x C x T x H x W ----------------
        NT, L, C = x.size()
...
class DTFBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        w_o = self.time_cor(x.float(), self.weights_cor)
        w_ = self.softmax(w_o)
        w_ = w_.transpose(1, 2)
        w_ = w_.reshape(B * T, self.group_num, 1, self.kernel_area, H, W)
        # spatial aggregation
        a_ = self.time_agg(x.transpose(1, 2).reshape(B * T, C, H, W).float(), w_)
...
class DTF_SWIN_ViT(C2D_SWIN_ViT):
    _arch_dict = {
        'dtf_swin_vit_s_p4_w7': dict(drop_path_rate=0.3, window_size=7, patch_size=4, in_chans=3, embed_dim=96,
                                     depths=[2, 2, 18, 2], num_heads=[3, 6, 12, 24], cor_dilation=1,
                                     blocks=[DTFBasicLayer, DTFBasicLayer, DTFBasicLayer, DTFBasicLayer]),

        'dtf_swin_vit_b_p4_w7': dict(drop_path_rate=0.5, window_size=7, patch_size=4, in_chans=3, embed_dim=128,
                                     depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32], cor_dilation=1,
                                     blocks=[DTFBasicLayer, DTFBasicLayer, DTFBasicLayer, DTFBasicLayer]),

        'dtf_swin_vit_l_p4_w7': dict(drop_path_rate=0.5, window_size=7, patch_size=4, in_chans=3, embed_dim=192,
                                     depths=[2, 2, 18, 2], num_heads=[6, 12, 24, 48], cor_dilation=1,
                                     blocks=[DTFBasicLayer, DTFBasicLayer, DTFBasicLayer, DTFBasicLayer])

    }

    @configurable
    def __init__(self, blocks=[], img_size=224, early_stride=4, patch_size=4, in_chans=3, num_classes=1000,
                 embed_dim=768, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24], window_size=7, mlp_ratio=4.,
                 qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, dropout_ratio=0.,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True, use_checkpoint=False, K=3, cor_dilation=1,
                 clip_len=4, weights='', transfer_weights=False, remove_fc=False):
        """
        Args:
        ...
        super().__init__(img_size, early_stride, patch_size, in_chans, num_classes, embed_dim, depths,
                         ...
                         drop_rate, attn_drop_rate, drop_path_rate, dropout_ratio, norm_layer, ape, patch_norm,
                         use_checkpoint, weights='', transfer_weights=False, remove_fc=False)
    ...
    def from_config(cls, cfg):
        ...
        region_kernel = arch_cfgs[0] if (len(arch_cfgs) > 0 and isinstance(arch_cfgs[0], int)) else None
        K = 3
        if region_kernel:
            K = region_kernel
        ...
        ret.update({
            'K': K,
            'clip_len': clip_length // cfg.MODEL.EARLY_STRIDE
        })
        return ret
