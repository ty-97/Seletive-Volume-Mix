...
class LGD_P3D_ResNet(BaseNet):
    _arch_dict = {
        # ResNet model from
        # `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
        'lgd_p3d_resnet18': dict(block=BasicBlock, layers=[2, 2, 2, 2]),
        'lgd_p3d_resnet34': dict(block=BasicBlock, layers=[3, 4, 6, 3]),
        'lgd_p3d_resnet50': dict(block=Bottleneck, layers=[3, 4, 6, 3]),
        'lgd_p3d_resnet101': dict(block=Bottleneck, layers=[3, 4, 23, 3]),
        'lgd_p3d_resnet103': dict(block=Bottleneck, layers=[3, 4, 23, 3], deep_stem=True),
        'lgd_p3d_resnet152': dict(block=Bottleneck, layers=[3, 8, 36, 3]),

        # ResNeXt model from
        # `"Aggregated Residual Transformation for Deep Neural Networks" <https://arxiv.org/pdf/1611.05431.pdf>`_
        'lgd_p3d_resnext50_32x4d': dict(block=Bottleneck, layers=[3, 4, 6, 3], groups=32, width_per_group=4),
        'lgd_p3d_resnext50_32x8d': dict(block=Bottleneck, layers=[3, 4, 23, 3], groups=32, width_per_group=8),
        ...
        'lgd_p3d_wide_resnet50_2': dict(block=Bottleneck, layers=[3, 4, 6, 3], width_per_group=64 * 2),
        'lgd_p3d_wide_resnet101_2': dict(block=Bottleneck, layers=[3, 4, 23, 3], width_per_group=64 * 2)
    }
    ...
    def __init__(self, block, layers, early_stride=4, num_classes=400, dropout_ratio=0.5, zero_init_residual=False,
        ...
        self.drop_dual = nn.Dropout(dropout_ratio, inplace=False)
    ...
    def _forward_impl(self, x, layer):
        ...
        # global path
        x_global = self.fc_dual(self.drop_dual(xg[1]))
