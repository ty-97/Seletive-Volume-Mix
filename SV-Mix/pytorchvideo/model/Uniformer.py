...
class Uniformer(BaseNet):
    _arch_dict = {
        'uniformer_s': dict(embed_dim = [64, 128, 320, 512],depth = [3, 4, 8, 3],mlp_ratio = 4,)
        # 'uniformer_b': dict()
    }
    ...
    def __init__(self, depth = [3, 4, 8, 3],
        ...
        drop_rate = 0.,
        attn_drop_rate = 0.,
        drop_path_rate = 0.2, #要调的
        split = False,
        std = False,
        use_checkpoint = False,
        checkpoint_num = [0, 0, 0, 0],
        weights='',
        transfer_weights=False, remove_fc=False
        ):
        super().__init__()
        # print('\n\n\n',drop_path_rate,'\n\n\n')
        ...
        # print('\n\n\n befload.py',self.blocks1[0].conv1.weight.shape,self.blocks1[0].conv1.weight[0,0])
        
        if weights != '':
            self.load_pretrained(weights, transfer_weights, remove_fc)
            
        # print('\n\n\n uni.py',self.blocks1[0].conv1.weight.shape,self.blocks1[0].conv1.weight[0,0])
    ...
    def load_pretrained(self, weights, transfer_weights, remove_fc):
        checkpoint = self.get_pretrained_model(weights)
        self.load_state_dict(checkpoint, strict=False)

    def get_pretrained_model(self, weight):
        # print(weight)
        if weight:
    ...
    def forward(self, x, featuremap=None):
        x = x[kfg.FRAMES]
        feature_map7x7 = self.forward_features(x)
        print(feature_map7x7.shape)
        x = feature_map7x7.flatten(2).mean(-1)
        x = self.head(x)
        # return x
