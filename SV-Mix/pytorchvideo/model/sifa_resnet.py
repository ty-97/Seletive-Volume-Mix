...
from pytorchvideo.layers.defcor import DefAgg, DefCorFixW
...
class SIFA_ResNet(BaseNet):
    _arch_dict = {
        # ResNet-50 Backbone
        'sifa_resnet50': dict(block=[Bottleneck, DEFLocalTransBottleneck, DEFLocalTransBottleneck,
                                     DEFLocalTransBottleneck], layers=[3, 4, 6, 3]),
        # ResNet-101 Backbone
        'sifa_resnet101': dict(block=[Bottleneck, DEFLocalTransBottleneck, DEFLocalTransBottleneck,
                                      DEFLocalTransBottleneck], layers=[3, 4, 23, 3])
    }

    @configurable
    def __init__(self, block, layers, num_classes=400, dropout_ratio=0.5, zero_init_residual=True,
        ...
        super(SIFA_ResNet, self).__init__()
        if norm_layer is None:
        ...
        if not deep_stem:
            self.conv1 = nn.Conv3d(3, self.inplanes, kernel_size=(4, 7, 7), stride=(4, 2, 2),
                                   padding=(0, 3, 3), bias=False)
            self.bn1 = norm_layer(self.inplanes)
            self.relu = nn.ReLU(inplace=True)
        else:
            self.conv1 = nn.Conv3d(3, self.inplanes, kernel_size=(4, 3, 3), stride=(4, 2, 2),
        ...
        self.fc_dual = nn.Linear(512 * block[0].expansion, num_classes)
    ...
    def from_config(cls, cfg):
        ...
        sifa_kernel = cfg.MODEL.ARCH_CFGS
        assert len(sifa_kernel) == 4, f'Expected 4 elements in cfg.MODEL.ARCH_CFGS for sifa_kernel in model ' \
                                      f'SIFA_ResNet. Got {len(sifa_kernel)} instead!'
        assert all(map(lambda x: isinstance(x, int), sifa_kernel)), f'All the elements in cfg.MODEL.ARCH_CFGS ' \
                                                                    f'should be int type!'
        ret.update({
            ...
            'sifa_kernel': sifa_kernel,
        })
        return ret

    @staticmethod
    def transfer_weights(state_dict, early_stride=4, *args, **kwargs):
        new_state_dict = {}
        for k, v in state_dict.items():
            v = v.detach().numpy()
            # remove def_cor weight
    ...
    def _return_layers(self, block, planes, blocks, stride=1, dilate=False, K_size=3, 
        ...
        if block == Bottleneck:
            layers = self._make_layer([block for i in range(blocks)], planes, blocks, stride=stride, dilate=dilate,
                                      K_size=K_size, dilation_size=dilation_size, group_num=group_num,
                                      clip_size=clip_size)
            return layers
        else:
            block_insert = (GBottleneck, block)
            layers = self._make_layer([block_insert[i % 2] for i in range(blocks)], planes, blocks, stride=stride,
                                      dilate=dilate, K_size=K_size, dilation_size=dilation_size, group_num=group_num,
                                      clip_size=clip_size)
            return layers 
    ...
    def forward(self, x):
        ...
        x_g_origin = self.pool(x_origin).flatten(1)
        x_g_dual = self.pool_dual(x_dual).flatten(1)
        ...
        if self.training:
            ...
            return ((x1_origin + x1_dual) / 2).softmax(dim=1)
