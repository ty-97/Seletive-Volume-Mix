...
from pytorchvideo.functional.dtf_layers.temporal_correlation import TemporalCorrelation
from pytorchvideo.functional.dtf_layers.temporal_aggregation import TemporalAggregation
...
class FFTMul(nn.Module):
    def __init__(self, dim, shape, bias=True):
        super().__init__()
        self.dim = dim
        self.shape = shape
        self.bias = bias
        weight_shape = [1, dim, shape[0], shape[1], shape[2], 2]
        self.complex_weight = nn.Parameter(torch.randn(weight_shape, dtype=torch.float32) * 0.02)
        if bias:
            bias_shape = [1, dim, 1, 1, 1, 2]
            self.complex_bias = nn.Parameter(torch.zeros(bias_shape, dtype=torch.float32))

    def extra_repr(self) -> str:
        return 'dim={}, shape={}, bias={}'.format(
            self.dim, self.shape, self.bias
        )
...
class DTFBlock(nn.Module):
    def __init__(self, dim, clip_len=16, K=3, factor_G=8):
        super().__init__()
        t = clip_len // 2 + 1
        ...
        self.filter_g = nn.Conv2d(in_channels=(dim+K*K)*self.clip_len, out_channels=dim*t*2//self.factor_G,
        ...
        self.filter2 = FFTMul(dim=dim, shape=[t, 1, 1], bias=False)
        self.linear2 = FFTLinear(in_features=dim, out_features=dim, bias=False)
        self.norm2 = FFTBatchNorm(dim=dim)

        self.alpha1 = nn.Parameter(torch.zeros([1, dim, 1, 1, 1]))
        self.alpha2 = nn.Parameter(torch.zeros([1, dim, 1, 1, 1]))
        ...
        self.weights_cor = torch.nn.Parameter(torch.ones(self.width, clip_len, K, K).to('cuda')*self._scale,
                                              requires_grad=False)
        self.time_cor = TemporalCorrelation(self.width, self.group_width, kernel_size=K, 
                                            ...
                                            group_size=self.cor_group_size)
    ...
    def forward(self, x):
        ...
        # global pooling
        yp = torch.mean(torch.mean(x_t, 3, True), 4, True)
        y2 = self.norm2(self.linear2(self.filter2(yp)))

        # reconstruct visual feature by IFFT
        out = torch.fft.irfft(y1 * self.alpha1 + y2 * self.alpha2, n=T, dim=2, norm='ortho')
        # skip connection
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, clip_length=None):
...
class DTF_ResNet(BaseNet):
    _arch_dict = {
        # ResNet-50 Backbone
        'dtf_resnet50': dict(block=Bottleneck, layers=[3, 4, 6, 3]),
        # ResNet-101 Backbone
        'dtf_resnet101': dict(block=Bottleneck, layers=[3, 4, 23, 3])
    }
    ...
    def __init__(self, block, layers, early_stride=4, num_classes=1000, dropout_ratio=0.5, zero_init_residual=False,
                 ...
                 clip_length=16, weights='', transfer_weights=False, remove_fc=False, frozen_bn=False):
        super(DTF_ResNet, self).__init__()
        if norm_layer is None:
        ...
        if not deep_stem:
            ...
            self.conv2 = nn.Conv3d(self.inplanes, self.inplanes, kernel_size=(1, 3, 3), stride=(1, 1, 1),
                                   padding=(0, 1, 1), bias=False)
            self.bn2 = norm_layer(self.inplanes)
            self.relu2 = nn.ReLU(inplace=True)
            self.conv3 = nn.Conv3d(self.inplanes, self.inplanes * 2, kernel_size=(1, 3, 3), stride=(1, 1, 1),
                                   padding=(0, 1, 1), bias=False)
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], clip_length=clip_length//early_stride)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0], clip_length=clip_length//early_stride)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1], clip_length=clip_length//early_stride)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2], clip_length=clip_length//early_stride)
        
        self.inplanes = 256 * block.expansion
        self.maxpool_dual = nn.MaxPool3d(kernel_size=(4, 1, 1), stride=(4, 1, 1), padding=(0, 0, 0))
        
        self.layer4_dual = self._make_layer(block, 512, layers[3], stride=1, dilate=replace_stride_with_dilation[2],
                                            clip_length=clip_length//(4 * early_stride))

        self.pool = nn.AdaptiveAvgPool3d(output_size=(1, 1, 1))
        self.pool_dual = nn.AdaptiveAvgPool3d(output_size=(1, 1, 1))

        self.drop = nn.Dropout(dropout_ratio)
        self.drop_dual = nn.Dropout(dropout_ratio)

        self.fc = nn.Linear(512 * block.expansion, num_classes)
        self.fc_dual = nn.Linear(512 * block.expansion, num_classes)
    ...
    def transfer_weights(state_dict, early_stride=None, *args, **kwargs):
        ...
        for k, v in state_dict.items():
            ...
            # convert fft weight
            if 'weights_cor' in k or 'freq_' in k:
                continue
    ...
    def _forward_impl(self, x, layer):
        ...
        x_g = self.pool(x_origin).flatten(1)
        x_g = self.drop(x_g)
        x1 = self.fc(x_g)

        x2_g = self.pool_dual(x_dual).flatten(1)
        x2_g = self.drop_dual(x2_g)
        x2 = self.fc_dual(x2_g)
    ...
    def forward(self, x, layer=7):
        ...
        if self.training:
            ...
            return ((y[0] + y[1]) / 2).softmax(dim=1)
